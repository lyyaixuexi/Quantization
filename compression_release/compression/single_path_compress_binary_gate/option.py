...
class Option(object):
    def __init__(self, conf_path):
        ...
        self.print_frequency = self.conf[
            "print_frequency"
        ...
        self.loss_lambda = self.conf["loss_lambda"]
        self.quantize_first_last = self.conf["quantize_first_last"]
        self.num_choices = self.conf["num_choices"]
        self.group_size = self.conf["group_size"]
        self.max_pruning_ratio = self.conf["max_pruning_ratio"]
        self.warmup_n_epochs = (
            self.conf["warmup_n_epochs"] if hasattr(self.conf, "warmup_n_epochs") else 0
        )

        # ------------- quantization options -----------------------------------
        self.qw = self.conf["qw"]
        self.qa = self.conf["qa"]
        self.bits_weights_list = self.conf["bits_weights_list"]
        self.bits_activations_list = self.conf["bits_activations_list"]
        self.from_scratch = (
            self.conf["from_scratch"] if hasattr(self.conf, "from_scratch") else False
        )

        # ---------- resume or pretrained options ---------------------------------
        # path to pretrained model, load model state_dict only
        self.pretrained = (
            None if len(self.conf["pretrained"]) == 0 else self.conf["pretrained"]
        )
        self.mixed_precision_pretrained = (
            None
            if len(self.conf["mixed_precision_pretrained"]) == 0
            else self.conf["mixed_precision_pretrained"]
        )
    ...
    def set_save_path(self):
        ...
        if self.quantize_first_last:
            first_last = "first_last"
        else:
            first_last = "nofirst_last"

        if self.net_type in ["preresnet", "resnet"]:
            self.save_path = (
                self.save_path
                + "log_{}{:d}_{}_w{}a{}_bs{:d}_e{:d}_lr{:.5f}_step{}_lambda{}_gp{}_maxp{}_{}/".format(
                    self.net_type,
                    self.depth,
                    self.dataset,
                    self.qw,
            ...
            self.save_path = (
                self.save_path
                + "log_{}_{}_w{}a{}_bs{:d}_e{:d}_lr{:.5f}_step{}_lambda{}_gp{}_maxp{}_{}/".format(
                    self.net_type,
                    self.dataset,
                    self.qw,
                    self.qa,
            ...
            for f in os.listdir(src):
                ...
                if len(file_split) >= 2 and file_split[1] == "py":
                    ...
                    try:
                        ...
                        logger.errro(
                            "copy file error! src: {}, dst: {}".format(
                                src_file, dst_file
                            )
                        )
