...
class Experiment(object):
    ...
    def __init__(self, options=None, conf_path=None):
        ...
        setup_tensorboard_logger_for_distributed(self.settings.rank == 0, self.tensorboard_logger)
    ...
    def _set_dataloader(self):
        ...
        if "cifar" in self.settings.dataset:
            ...
            self.train_loader, self.val_loader, self.train_sampler, self.val_sampler = get_imagenet_dataloader(
    ...
    def _load_resume(self):
        ...
        if self.settings.resume is not None:
            model_state, optimizer_state, epoch, lr_scheduler_state = self.checkpoint.load_checkpoint(
    ...
    def run(self):
        self.trainer.init_scale()
        _, _, _ = self.trainer.val(0)
        self.checkpoint.save_model(self.model, best_flag=True)
