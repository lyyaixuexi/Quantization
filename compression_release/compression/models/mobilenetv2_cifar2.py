...
__all__ = ["MobileNetV2CIFAR2", "MobileBottleneck"]
...
class MobileBottleneck(nn.Module):
    def __init__(self, in_planes, out_planes, expand=1, stride=1):
        ...
        self.conv2 = dwconv3x3(intermedia_planes, intermedia_planes, stride=stride)
        ...
        if self.shortcut:
            self.downsample = nn.Sequential(
                conv1x1(in_planes, out_planes), nn.BatchNorm2d(out_planes)
            )
        else:
...
class MobileNetV2CIFAR2(nn.Module):
    ...
    def __init__(self, num_classes=1000, wide_scale=1.0):
        ...
        self.conv2 = conv1x1(in_planes=self.layer_width[7].item(), out_planes=1280)
