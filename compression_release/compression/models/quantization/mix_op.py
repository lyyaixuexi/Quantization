...
def build_candidate_ops_w_diff_out_planes(in_planes, out_planes, stride=1):
    out_channels_list = []
    for ratio in ratios:
        out_channels_list.append(int(out_planes * ratio))
    out_channels_list = sorted( list( set(out_channels_list) ) )

    return [
        conv3x3(in_planes, out_channels, stride) for out_channels in out_channels_list
    ]

def build_candidate_ops_w_diff_in_planes(in_planes, out_planes, stride=1):
    in_channels_list = []
    for ratio in ratios:
        in_channels_list.append(int(in_planes * ratio))
    in_channels_list = sorted( list( set(in_channels_list) ) )

    return [
        conv3x3(in_channels, out_planes, stride) for in_channels in in_channels_list
    ]
...
class MixedEdge(nn.Module):
    ...
    def __init__(self, candidate_ops):
        ...
        self.AP_path_alpha = nn.Parameter(torch.Tensor(self.n_choices))  # architecture parameters
        self.AP_path_wb = nn.Parameter(torch.Tensor(self.n_choices))  # binary gates
