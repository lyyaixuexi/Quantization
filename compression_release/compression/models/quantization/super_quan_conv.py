...
def reduce_quantize_activation(
    ...
    if is_train:
        ...
        for i in range(len(bits_activations_list) - 1):
            ...
            # l1-norm
            residual_error_norm = residual_error.abs().sum() / (output.nelement())
            residual_error_norm = AllReduce.apply(residual_error_norm) * (
                1.0 / dist.get_world_size()
            )
            # l2-norm
            # residual_error_norm = (residual_error ** 2).sum() / (output.nelement())
            indicator = (
...
def normalize_and_quantize_weight(x, bits_weights, clip_value):
    x = normalization_on_weights(x, clip_value)
    x = (x + 1.0) / 2.0
...
class SuperQConv2d(nn.Conv2d):
    ...
    def __init__(
        ...
        self.register_buffer(
            "bits_weights", torch.FloatTensor([self.bits_weights_list[0]])
        )
        self.register_buffer(
            "bits_activations", torch.FloatTensor([self.bits_activations_list[0]])
        )
    ...
    def fix_weight_compute_activation_bops(self):
        if self.weight_indicator_list[0] == 0.0:
            self.bits_weights.data.fill_(self.bits_weights_list[0])
        else:
            if len(self.weight_indicator_list) == 1:
                if self.weight_indicator_list[0] == 1.0:
                    self.bits_weights.data.fill_(self.bits_weights_list[1])
            else:
                if (
                    ...
                    self.bits_weights.data.fill_(self.bits_weights_list[2])
        ...
        if len(input_indicator_.shape) == 0:
            input_indicator = torch.cat(
                [input_indicator_.new_tensor([1.0]), input_indicator_.unsqueeze(0)]
            )
        else:
            input_indicator = torch.cat(
    ...
    def fix_activation_compute_weight_bops(self):
        if self.input_indicator_list[0] == 0:
            self.bits_activations.data.fill_(self.bits_activations_list[0])
        else:
            if len(self.input_indicator_list) == 1:
                if self.input_indicator_list[0] == 1:
                    self.bits_activations.data.fill_(self.bits_activations_list[1])
            else:
                if (
                    self.input_indicator_list[0] == 1
                    and self.input_indicator_list[1] == 0
                ):
                    self.bits_activations.data.fill_(self.bits_activations_list[1])
                elif (
                    self.input_indicator_list[0] == 1
                    and self.input_indicator_list[1] == 1
                ):
                    self.bits_activations.data.fill_(self.bits_activations_list[2])
        ...
        if len(weight_indicator_.shape) == 0:
            weight_indicator = torch.cat(
                [weight_indicator_.new_tensor([1.0]), weight_indicator_.unsqueeze(0)]
            )
        else:
            weight_indicator = torch.cat(
    ...
    def compute_current_bops(self, channel_num, is_out_channel=True):
        ...
        if self.weight_indicator_list[0] == 0.0:
            ...
            if len(self.weight_indicator_list) == 1:
                if self.weight_indicator_list[0] == 1:
                    self.bits_weights.data.fill_(self.bits_weights_list[1])
            else:
                if (
                    self.weight_indicator_list[0] == 1.0
...
class SuperQWeightConv2d(nn.Conv2d):
    """
    custom convolutional layers for quantization
    """
    ...
    def __init__(
        ...
        super(SuperQWeightConv2d, self).__init__(
            in_channels,
            out_channels,
            kernel_size,
            stride,
    ...
    def forward(self, *args):
        if len(args) == 1:
            input = args[0]
            quantized_input = dorefa_clip.quantize_activation(
    ...
    def fix_activation_pre_compute_weight_bops(self, bit_activations):
        kernel_size = self.kernel_size[0]
        if bit_activations == 32:
            bit_activations = bit_activations.new_tensor([8.0])
        for i, bits_weights in enumerate(self.bits_weights_list):
            self.weight_bops[i] = compute_bops(
                kernel_size,
                self.in_channels,
...
class SuperQLinear(nn.Linear):
    ...
    def __init__(
        ...
        super(SuperQLinear, self).__init__(in_features, out_features, bias=bias)
        # self.eps = 1e-5
