...
class QConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        quantized_weight = quantized_weight / torch.sqrt(torch.var(quantized_weight.detach()*quantized_weight.shape[0]))
        output = F.conv2d(
            quantized_input,
            quantized_weight,
            self.bias,
    ...
    def extra_repr(self):
        ...
        s += ", method={}".format("dorefa_clip_rcf_std_conv")
        return s
...
class QLinear(nn.Linear):
    ...
    def extra_repr(self):
        ...
        s += ", bits_weights={}".format(self.bits_weights)
        s += ", bits_activations={}".format(self.bits_activations)
        s += ", method={}".format("dorefa_rcf_std_linear")
        return s
