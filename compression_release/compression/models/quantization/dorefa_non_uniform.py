...
class StepFunction(torch.autograd.Function):
    ...
    def backward(self, grad_output):
        ...
        return grad_input, (-grad_input).sum(), None 
...
def quantization(x, k, b, T):
    ...
    output = 0
    for k in range(n):
        # left_value = (b[k] + b[k-1]) / 2.0 if k != 0 else 0
        # right_value = (b[k] + b[k + 1]) / 2.0 if k != n - 1 else 1
        # distance = b[k] - b[]
...
class QConv2d(nn.Conv2d):
    ...
    def extra_repr(self):
        ...
        s += ", T={}".format(self.T)
        s += ", method={}".format("dorefa_wn_non_uniform")
        return s
