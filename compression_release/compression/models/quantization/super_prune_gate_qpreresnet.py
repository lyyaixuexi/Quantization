...
from compression.models.quantization.dorefa_clip_prune import QConv2d, QLinear
...
class SuperPrunedGateQPreBasicBlock(nn.Module):
    """
    ...
    def __init__(
        ...
        """
        super(SuperPrunedGateQPreBasicBlock, self).__init__()
    ...
    def get_bops(self):
        ...
        conv1_bops = compute_bops(
            ...
            self.output_h[0],
            self.output_w[0],
            self.conv1.bits_weights,
            self.conv1.bits_activations,
        )
        conv2_bops = compute_bops(
            ...
            self.output_h[1],
            self.output_w[1],
            self.conv2.bits_weights,
            self.conv2.bits_activations,
        )
        downsample_bops = 0
        if self.downsample is not None:
            downsample_bops = compute_bops(
                ...
                self.downsample.h,
                self.downsample.w,
                self.downsample.bits_weights,
                self.downsample.bits_activations,
            )
    ...
    def get_param_size(self):
        ...
        total_params = (
            bn1_params + conv1_params + bn2_params + conv2_params + downsample_params
        )
    ...
    def forward(self, x):
        ...
        if not self.init_state:
            ...
            if self.name == "half_preact":
                ...
                indicator_reshape = indicator_reshape.expand(
                ...
                x = self.conv1(x, weight_mean, weight_std)
                x = self.bn2(x)
                x = self.relu2(x)

                n, c, h, w = x.shape
                reshape_x = x.reshape(n, c // self.group_size, self.group_size, h, w)
                reshape_x = reshape_x * indicator.unsqueeze(0).unsqueeze(-1).unsqueeze(
                    -1
                ).unsqueeze(-1)
                ...
                weight_mean = selected_weight.data.mean()
                weight_std = selected_weight.data.std()
                x = self.conv2(x, weight_mean, weight_std)
            elif self.name == "both_preact":
                residual = x
                x = self.bn1(x)
                x = self.relu1(x)
    ...
    def extra_repr(self):
        s = super().extra_repr()
        s += ", method={}".format("SuperPrunedGateQPreBasicBlock")
        return s


class SuperPrunedGateQPreResNet(nn.Module):
    """
    define SuperPreResNet on small data sets
    """

    def __init__(self, depth, wide_factor=1, num_classes=10, group_size=4):
        """
        ...
        """
        super(SuperPrunedGateQPreResNet, self).__init__()
        ...
        self.layer1 = self._make_layer(
            SuperPrunedGateQPreBasicBlock, 16 * wide_factor, n, group_size=group_size
        )
        self.layer2 = self._make_layer(
            SuperPrunedGateQPreBasicBlock,
            32 * wide_factor,
            n,
            stride=2,
    ...
    def _make_layer(self, block, out_plane, n_blocks, stride=1, group_size=4):
        """
        ...
        for i in range(1, int(n_blocks)):
            layers.append(block(self.in_plane, out_plane, group_size=group_size))
    ...
    def get_bops(self):
        ...
        for i in range(3):
            ...
            for name, module in layer.named_modules():
                if isinstance(module, SuperPrunedGateQPreBasicBlock):
                    bops = module.get_bops()
                    current_bops += bops

        layer = self.fc
        bops = compute_bops(
            1, layer.in_features, layer.out_features, 1, 1, bits_w=8, bits_a=8,
        )
        current_bops += bops
        return current_bops
