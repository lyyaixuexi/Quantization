...
from compression.models.quantization.super_joint_quan_conv import SuperQConv2d
from compression.utils.utils import *

__all__ = [
    "SuperCompressedGateResNet",
...
class SuperCompressedJointGateBasicBlock(nn.Module):
    expansion = 1
    ...
    def __init__(
        ...
        super(SuperCompressedJointGateBasicBlock, self).__init__()
    ...
    def get_downsample_bops(self):
        ...
        if self.downsample is not None:
            downsample_bops = compute_bops(self.downsample[0].kernel_size[0], self.downsample[0].in_channels, 
                                                self.downsample[0].out_channels, self.downsample[0].h, self.downsample[0].w)
    ...
    def compute_bops(self):
        ...
        # bops of conv1
        conv1_bops = self.conv1.give_channel_num_compute_bops(channel_num, is_out_channel=True)
        # bops of conv2
        conv2_bops = self.conv2.give_channel_num_compute_bops(channel_num, is_out_channel=False)
        downsample_bops = 0
        if self.downsample is not None:
            downsample_bops = self.downsample.compute_bops()
...
class SuperCompressedJointGateBottleneck(nn.Module):
    expansion = 4
    ...
    def __init__(
        ...
        super(SuperCompressedJointGateBottleneck, self).__init__()
        ...
        self.register_buffer("assigned_indicator_1", torch.zeros(self.num_choices))
        self.indicator_1 = None
        self.channel_thresholds_2 = nn.Parameter(torch.zeros(1))
        self.register_buffer("assigned_indicator_2", torch.zeros(self.num_choices))
    ...
    def get_total_bops(self):
        conv1_bops = compute_bops(self.conv1.kernel_size[0], self.conv1.in_channels, self.conv1.out_channels, self.output_h[0], self.output_w[0])
        conv2_bops = compute_bops(self.conv2.kernel_size[0], self.conv2.in_channels, self.conv2.out_channels, self.output_h[1], self.output_w[1])
        conv3_bops = compute_bops(self.conv3.kernel_size[0], self.conv3.in_channels, self.conv3.out_channels, self.output_h[2], self.output_w[2])
    ...
    def forward(self, x):
        if not self.init_state:
            ...
            indicator_1 = indicator_1.expand(n // self.group_size, self.group_size).reshape(n)
            ...
            indicator_2 = indicator_2.expand(n // self.group_size, self.group_size).reshape(n)
            index_2 = (indicator_2 > 0).nonzero().squeeze()
            selected_weight = torch.index_select(self.conv2.weight, 1, index_1)
            selected_weight = torch.index_select(self.selected_weight, 0, index_2)
            weight_mean = selected_weight.data.mean()
            weight_std = selected_weight.data.std()

            out = out * indicator_1.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)

            out = self.conv2(out, self.weight, weight_mean, weight_std)
            out = self.bn2(out)
            out = self.relu(out)
            ...
            out = self.conv3(out, self.weight, weight_mean, weight_std)
            out = self.bn3(out)
...
class SuperCompressedJointGateResNet(nn.Module):
    def __init__(self, depth, num_classes=1000, num_choices=8):
        self.inplanes = 64
        super(SuperCompressedJointGateResNet, self).__init__()
        if depth < 50:
            block = SuperCompressedJointGateBasicBlock
        else:
            block = SuperCompressedJointGateBasicBlock
    ...
    def _make_layer(
        self, block, planes, blocks, stride=1, num_choices=8
    ):
        downsample = None
