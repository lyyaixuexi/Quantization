...
class SuperQConv2d(nn.Conv2d):
    ...
    def __init__(
        ...
        for i in range(len(self.bits_weights_list)):
            self.weight_memory_footprint.append(0)
        self.activation_memory_footprint = []
        for i in range(len(self.bits_activations_list)):
            self.activation_memory_footprint.append(0)
    ...
    def pre_compute_memory_footprint(
        self,
        given_bit_weights,
        given_bit_activation,
        channel_num,
        is_compute_weights_memory_footprint=True,
        is_out_channel=True,
    ):
        if is_out_channel:
        ...
        if is_compute_weights_memory_footprint:
            activation_memory_footprint = compute_memory_footprint(1, in_channels, self.h, self.w, given_bit_activation)
            for i, bits_weights in enumerate(self.bits_weights_list):
                _, _, weight_h, weight_w = self.weight.shape
                weight_memory_footprint = compute_memory_footprint(out_channels // self.groups, in_channels, weight_h, weight_w, bits_weights)
                self.weight_memory_footprint[i] = weight_memory_footprint + activation_memory_footprint
            for i in range(len(self.bits_weights_list) - 1, -1, -1):
                if i != 0:
                    self.weight_memory_footprint[i] = self.weight_memory_footprint[i] - self.weight_memory_footprint[i - 1]
        else:
            _, _, weight_h, weight_w = self.weight.shape
            weight_memory_footprint = compute_memory_footprint(out_channels // self.groups, in_channels, weight_h, weight_w, given_bit_weights)
            for i, bits_activations in enumerate(self.bits_activations_list):
                activation_memory_footprint = compute_memory_footprint(1, in_channels, self.h, self.w, bits_activations)
                self.activation_memory_footprint[i] = weight_memory_footprint + activation_memory_footprint
            for i in range(len(self.bits_weights_list) - 1, -1, -1):
                if i != 0:
                    self.activation_memory_footprint[i] = (
                        self.activation_memory_footprint[i] - self.activation_memory_footprint[i - 1]
                    )

    def pre_compute_channel_memory_footprint(
        self,
        given_bit_weights,
        given_bit_activation,
        in_channels,
    ...
    def fix_activation_pre_compute_weight_memory_footprint(self, bit_activations):
        activation_memory_footprint = compute_memory_footprint(1, self.c, self.h, self.w, bit_activations)
        for i, bits_weights in enumerate(self.bits_weights_list):
            weight_n, weight_c, weight_h, weight_w = self.weight.shape
            weight_memory_footprint = compute_memory_footprint(weight_n, weight_c, weight_h, weight_w, bits_weights)
            self.weight_memory_footprint[i] = weight_memory_footprint + activation_memory_footprint
        for i in range(len(self.bits_weights_list) - 1, -1, -1):
            if i != 0:
                self.weight_memory_footprint[i] = self.weight_memory_footprint[i] - self.weight_memory_footprint[i - 1]

    def fix_weight_pre_compute_activation_memory_footprint(self, bit_weights):
        weight_n, weight_c, weight_h, weight_w = self.weight.shape
        weight_memory_footprint = compute_memory_footprint(weight_n, weight_c, weight_h, weight_w, bit_weights)
        for i, bits_activations in enumerate(self.bits_activations_list):
    ...
    def fix_weight_compute_activation_memory_footprint(self):
        if self.num_bits == 3:
            ...
            if self.weight_indicator_list[0] == 0.0:
                ...
                self.bits_weights.data.fill_(8)
        self.fix_weight_pre_compute_activation_memory_footprint(self.bits_weights)
        ...
        activation_memory_footprint = input_indicator_.new_tensor(self.activation_memory_footprint)
        memory_footprint = (torch.cumprod(input_indicator, dim=0) * activation_memory_footprint).sum()
        return memory_footprint

    def compress_fix_weight_compute_activation_memory_footprint(
        ...
        self.pre_compute_memory_footprint(
            ...
            8,
            channel_num,
            is_compute_weights_memory_footprint=False,
    ...
    def compress_channel_fix_weight_compute_activation_memory_footprint(
        ...
        if self.num_bits == 3:
            ...
            if self.weight_indicator_list[0] == 0.0:
                ...
                self.bits_weights.data.fill_(8)
        self.pre_compute_channel_memory_footprint(
            self.bits_weights,
            8,
            in_channels,
            out_channels,
    ...
    def fix_activation_compute_weight_memory_footprint(self):
        if self.num_bits == 3:
            ...
            if self.input_indicator_list[0] == 0:
                ...
                self.bits_activations.data.fill_(8)
        self.fix_activation_pre_compute_weight_memory_footprint(self.bits_activations)
        ...
        weight_memory_footprint = weight_indicator_.new_tensor(self.weight_memory_footprint)
        memory_footprint = (torch.cumprod(weight_indicator, dim=0) * weight_memory_footprint).sum()
        return memory_footprint

    def compress_fix_activation_compute_weight_memory_footprint(
        self, channel_num, is_out_channel=True
    ):
        if self.num_bits == 3:
            if self.input_indicator_list[0] == 0:
    ...
    def compress_channel_fix_activation_compute_weight_memory_footprint(
        self, in_channels, out_channels
    ):
        if self.num_bits == 3:
            if self.input_indicator_list[0] == 0:
    ...
    def compute_current_memory_footprint(self, channel_num, is_out_channel=True):
        if self.input_indicator_list[0] == 0:
            self.bits_activations.data.fill_(2)
        elif self.input_indicator_list[0] == 1 and self.input_indicator_list[1] == 0:
            self.bits_activations.data.fill_(4)
        ...
        memory_footprint = compute_memory_footprint(
            kernel_size, 
            in_channels,
