...
from compression.models.quantization.dq_asy_conv import DQConv2d, DQLinear
...
class SqueezeBlock(nn.Module):
    def __init__(self, exp_size, divide=4):
        ...
        self.dense = nn.Sequential(
            DQLinear(exp_size, exp_size // divide),
            nn.ReLU(inplace=True),
            DQLinear(exp_size // divide, exp_size),
            h_sigmoid()
        )
...
class DQAsyMobileBlockCifar(nn.Module):
    def __init__(self, in_channels, out_channels, kernal_size, stride, nonLinear, SE, exp_size):
        ...
        if self.expand:
            self.conv1 = DQConv2d(in_channels, exp_size, kernel_size=1, stride=1, padding=0, bias=False)
            self.bn1 = nn.BatchNorm2d(exp_size)
            self.relu1 = activation(inplace=True)

        self.conv2 = DQConv2d(exp_size, exp_size, kernel_size=kernal_size, stride=stride, padding=padding, groups=exp_size, bias=False)
        self.bn2 = nn.BatchNorm2d(exp_size)
        self.relu2 = activation(inplace=True)

        self.conv3 = DQConv2d(exp_size, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
...
class DQAsyMobileNetV3Cifar(nn.Module):
    def __init__(self, model_mode="large", num_classes=1000, multiplier=1.0, dropout_rate=0.2, quantize_first_last=False):
        super(DQAsyMobileNetV3Cifar, self).__init__()
        ...
        for kernel_size, exp_size, out_channels_num, use_SE, NL, stride in layers:
            ...
            self.block.append(DQAsyMobileBlockCifar(init_conv_out, output_channels_num, kernel_size, stride, NL, use_SE, exp_size))
        ...
        self.conv2 = DQConv2d(out_conv1_in, out_conv1_out, kernel_size=1, stride=1, bias=False)
        ...
        self.conv3 = DQConv2d(out_conv2_in, out_conv2_out, kernel_size=1, stride=1)
...
# model = DQAsyMobileNetV3Cifar(model_mode="LARGE", num_classes=1000, multiplier=1.0)
