...
from compression.models.quantization.super_quan_asy_conv import SuperAsyQConv2d, SuperAsyQLinear
...
class SqueezeBlock(nn.Module):
    def __init__(self, exp_size, divide=4):
        ...
        self.dense = nn.Sequential(
            SuperAsyQLinear(exp_size, exp_size // divide),
            nn.ReLU(inplace=True),
            SuperAsyQLinear(exp_size // divide, exp_size),
            h_sigmoid()
        )
...
class SuperCompressAsyMobileBlockCifar(nn.Module):
    def __init__(self, in_channels, out_channels, kernal_size, stride, nonLinear, SE, exp_size, group_size=16):
        super(SuperCompressAsyMobileBlockCifar, self).__init__()
        ...
        if self.expand:
            self.conv1 = SuperAsyQConv2d(in_channels, exp_size, kernel_size=1, stride=1, padding=0, bias=False)
            self.bn1 = nn.BatchNorm2d(exp_size)
            self.relu1 = activation(inplace=True)

        self.conv2 = SuperAsyQConv2d(exp_size, exp_size, kernel_size=kernal_size, stride=stride, padding=padding, groups=exp_size, bias=False)
        self.bn2 = nn.BatchNorm2d(exp_size)
        self.relu2 = activation(inplace=True)

        self.conv3 = SuperAsyQConv2d(exp_size, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def se_fix_activation_compute_weight_bops(self, channel_num):
        if self.SE:
            return self.squeeze_block.dense[0].compress_fix_activation_compute_weight_bops(channel_num=channel_num, is_out_channel=False) + self.squeeze_block.dense[2].compress_fix_activation_compute_weight_bops(channel_num=channel_num, is_out_channel=True)
        else:
            return 0

    def se_fix_weight_compute_activation_bops(self, channel_num):
        if self.SE:
            return self.squeeze_block.dense[0].compress_fix_weight_compute_activation_bops(channel_num=channel_num, is_out_channel=False) + self.squeeze_block.dense[2].compress_fix_weight_compute_activation_bops(channel_num=channel_num, is_out_channel=True)
        else:
            return 0
    ...
    def compress_fix_activation_compute_weight_bops(self):
        ...
        se_bops = self.se_fix_activation_compute_weight_bops(channel_num)
    ...
    def compress_fix_weight_compute_activation_bops(self):
        ...
        se_bops = self.se_fix_weight_compute_activation_bops(channel_num)
        total_bops = conv1_bops + conv2_bops + conv3_bops + se_bops
        return total_bops
    ...
    def forward(self, x):
        if not self.init_state:
            ...
            out = self.conv2(out, selected_weight)
            out = self.bn2(out)

            # Squeeze and Excite
            if self.SE:
                channel_num = out.shape[1]
                mask = out.new_zeros(channel_num)
                mask = torch.index_fill(mask, 0, index, 1).reshape(1, channel_num, 1, 1)
                out = out * mask
                out = self.squeeze_block(out)
            else:
                out = self.relu2(out)
            ...
            out = self.conv3(out, selected_weight)
            out = self.bn3(out)
...
class SuperCompressAsyMobileNetV3Cifar(nn.Module):
    def __init__(self, model_mode="large", num_classes=1000, multiplier=1.0, dropout_rate=0.2, bits_weights=32, bits_activations=32, quantize_first_last=False, group_size=16):
        super(SuperCompressAsyMobileNetV3Cifar, self).__init__()
        ...
        for kernel_size, exp_size, out_channels_num, use_SE, NL, stride in layers:
            ...
            self.block.append(SuperCompressAsyMobileBlockCifar(init_conv_out, output_channels_num, kernel_size, stride, NL, use_SE, exp_size, group_size=group_size))
        ...
        self.conv2 = SuperAsyQConv2d(out_conv1_in, out_conv1_out, kernel_size=1, stride=1, bias=False)
        ...
        self.conv3 = SuperAsyQConv2d(out_conv2_in, out_conv2_out, kernel_size=1, stride=1)
...
# model = SuperCompressAsyMobileNetV3Cifar(model_mode="LARGE", num_classes=1000, multiplier=1.0)
