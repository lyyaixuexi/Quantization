...
class SqueezeBlock(nn.Module):
    def __init__(self, exp_size, divide=4, bits_weights=32, bits_activations=32):
        super(SqueezeBlock, self).__init__()
        self.dense = nn.Sequential(
            QLinear(exp_size, exp_size // divide, bits_weights=bits_weights, bits_activations=bits_activations),
            nn.ReLU(inplace=True),
            QLinear(exp_size // divide, exp_size, bits_weights=bits_weights, bits_activations=bits_activations),
            h_sigmoid()
        )
...
class QAsyMobileBlockCifar(nn.Module):
    def __init__(self, in_channels, out_channels, kernal_size, stride, nonLinear, SE, exp_size, bits_weights=32, bits_activations=32):
        ...
        if self.SE:
            self.squeeze_block = SqueezeBlock(exp_size, bits_weights=bits_weights, bits_activations=bits_activations)
...
class QAsyMobileNetV3Cifar(nn.Module):
    def __init__(self, model_mode="large", num_classes=1000, multiplier=1.0, dropout_rate=0.2, bits_weights=32, bits_activations=32, quantize_first_last=False):
        super(QAsyMobileNetV3Cifar, self).__init__()
        ...
        for kernel_size, exp_size, out_channels_num, use_SE, NL, stride in layers:
            ...
            self.block.append(QAsyMobileBlockCifar(init_conv_out, output_channels_num, kernel_size, stride, NL, use_SE, exp_size, bits_weights=bits_weights, bits_activations=bits_activations))
...
# model = QAsyMobileNetV3Cifar(model_mode="LARGE", num_classes=1000, multiplier=1.0)
