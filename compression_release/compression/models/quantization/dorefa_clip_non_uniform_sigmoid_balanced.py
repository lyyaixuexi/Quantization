...
def step_backward_sigmoid(x, b, endpoints_T, n):
    # b_buf = x - b
...
def quantization(x, k, b, T):
    ...
    with torch.no_grad():
        ...
        interval_unique_index = torch.arange(0, n, device=x.device)
        each_interval_num = (interval_unique_index.unsqueeze(-1) == interval_index.reshape(-1)).long().sum(-1).float()

    b = gradient_scale_function(b, 1.0 / (each_interval_num * n).sqrt())
