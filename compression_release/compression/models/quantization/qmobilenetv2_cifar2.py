...
from compression.models.quantization.dorefa_clip_std import QConv2d, QLinear

__all__ = ["QMobileNetV2CIFAR2", "QMobileBottleneck"]
...
def qconv1x1(in_planes, out_planes, bits_weights=32, bits_activations=32):
    "1x1 convolution"
    return QConv2d(in_planes, out_planes, kernel_size=1, bias=False, 
                   bits_weights=bits_weights,
                   bits_activations=bits_activations)
...
class QMobileBottleneck(nn.Module):
    def __init__(self, in_planes, out_planes, expand=1, stride=1, bits_weights=32, bits_activations=32):
        ...
        if self.expand != 1:
            self.conv1 = qconv1x1(in_planes, intermedia_planes, 
                                  bits_weights=bits_weights, bits_activations=bits_activations)
            self.bn1 = nn.BatchNorm2d(intermedia_planes)
            self.relu1 = nn.ReLU6(inplace=True)

        self.conv2 = qdwconv3x3(intermedia_planes, intermedia_planes, stride=stride,
                                bits_weights=bits_weights, bits_activations=bits_activations)
        self.bn2 = nn.BatchNorm2d(intermedia_planes)
        self.relu2 = nn.ReLU6(inplace=True)

        self.conv3 = qconv1x1(intermedia_planes, out_planes,
                              bits_weights=bits_weights, bits_activations=bits_activations)
...
class QMobileNetV2CIFAR2(nn.Module):
    """
    MobileNet_v2
    """

    def __init__(self, num_classes=1000, wide_scale=1.0, bits_weights=32, bits_activations=32):
        super(QMobileNetV2CIFAR2, self).__init__()
        ...
        self.conv1 = conv3x3(3, self.in_planes, stride=1)
        self.bn1 = nn.BatchNorm2d(self.in_planes)
        self.relu1 = nn.ReLU6(inplace=True)
        self.layer1 = self._make_layer(block, self.layer_width[1].item(), blocks=1,
                                       bits_weights=bits_weights, bits_activations=bits_activations)
        self.layer2 = self._make_layer(
            block, self.layer_width[2].item(), blocks=2, expand=6, stride=1,
            bits_weights=bits_weights, bits_activations=bits_activations
        )
        ...
        self.conv2 = qconv1x1(in_planes=self.layer_width[7].item(), out_planes=1280, 
                              bits_weights=bits_weights, bits_activations=bits_activations)
    ...
    def _make_layer(self, block, out_planes, blocks, expand=1, stride=1, bits_weights=32, bits_activations=32):
        layers = []
        layers.append(block(self.in_planes, out_planes, expand=expand, stride=stride,
                            bits_weights=bits_weights, bits_activations=bits_activations))
