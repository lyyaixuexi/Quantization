...
def quantize_activation(x, step_size, clip_value, bias):
    x = clamp(x - bias, min=x.new_tensor([0]), max=clip_value)
    return step_size * RoundFunction.apply(x / step_size) + bias
...
class DQConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        quantized_input = quantize_activation(input, self.a_s, self.activation_clip_value, self.activation_bias)
    ...
    def compute_bops(self):
        ...
        bops = compute_bops(self.kernel_size[0], self.in_channels, self.out_channels // self.groups, self.h, self.w, bits_weights, bits_activations)
        return bops
    ...
    def extra_repr(self):
        ...
        s += ", method={}".format("dq_asy_conv")
        return s


class DQLinear(nn.Linear):
    """
    custom convolutional layers for quantization
    """
    ...
    def compute_bops(self):
        bits_weights, bits_activations = self.compute_bits()
        bops = compute_bops(1, self.in_features, self.out_features, 1, 1, bits_weights, bits_activations)
        return bops
