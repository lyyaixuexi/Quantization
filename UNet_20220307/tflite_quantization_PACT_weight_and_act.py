...
class Conv2d_quantization(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.act_max = nn.Parameter(torch.ones(1)*6)
...
# replace model
def replace(model, quantization_bits=8, m_bits=12, bias_bits=16, inference_type="all_fp", Mn_aware=False, fuseBN=False, layer_bit_dict=None):
    # 遍历整个模型，把原始模型的卷积层替换为自定义的模拟量化卷积层
    if layer_bit_dict is None:
        count = 0
        for name, module in model.named_modules():
            bits_act = 8 if count == 0 and quantization_bits<6 else quantization_bits
            bits_weight = 8 if count in [0,18] and quantization_bits<6 else quantization_bits
