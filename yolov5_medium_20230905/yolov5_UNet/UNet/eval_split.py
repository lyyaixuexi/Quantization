...
feature_map_dict = {}
def hook_conv_results_histogram(module, input, output):
    print("conv_idx:{}  output.size():{}   min:{}  max:{}".format(
        module.conv_idx, output.size(), output.min(), output.max()))

    if CHANNLE_EACH_GROUP == -1:
        if hasattr(module, "number_of_accumulate_batchs"):
            module.number_of_accumulate_batchs += 1
        else:
            module.number_of_accumulate_batchs = 0
            feature_map_dict[module.conv_idx] = output.data.cpu()

        if module.number_of_accumulate_batchs < 10:
            feature_map_dict[module.conv_idx] = torch.cat([feature_map_dict[module.conv_idx], output.data.cpu()], dim=0)

            hist_y = (torch.histc(input=output.data.cpu(), bins=100) / output.data.numel()).cpu()  # 百分比直方图
            hist_x = np.array([(output.max() - output.min()) / 100 * i + output.min() for i in range(100)])
            plt.plot(hist_x, hist_y, color="red")
            save_path = os.path.join(HIST_SAVE_DIR, str(module.conv_idx))
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            save_path = os.path.join(save_path, "batch_"+str(module.number_of_accumulate_batchs) + ".png")
            plt.savefig(save_path)
            plt.clf()

        elif module.number_of_accumulate_batchs == 10:
            hist_y = (torch.histc(input=feature_map_dict[module.conv_idx] , bins=100) / feature_map_dict[module.conv_idx].numel()).cpu() # 百分比直方图
            hist_x = np.array([(output.max()-output.min())/100*i+output.min() for i in range(100)])
            plt.plot(hist_x, hist_y, color="red")
            save_path = os.path.join(HIST_SAVE_DIR, str(module.conv_idx) + "_total.png")
            plt.savefig(save_path)
            plt.clf()
        else:
            pass
        ...
        if module.number_of_accumulate_batchs < 10:
            with torch.no_grad():
                incomplete_group_size = module.in_channels % CHANNLE_EACH_GROUP # 通道数可能不能整除 CHANNLE_EACH_GROUP
                group_number = module.in_channels // CHANNLE_EACH_GROUP
                if incomplete_group_size == 0:
                    print("CHANNLE_EACH_GROUP:{} group_number:{}".format(CHANNLE_EACH_GROUP, group_number))
                else:
                    print("CHANNLE_EACH_GROUP:{} group_number:{} addition incomplete_group_size:{}".format(CHANNLE_EACH_GROUP, group_number, incomplete_group_size))

                output_split_sum = torch.zeros_like(output)
                for i in range(group_number):
                    affine_idx = int(i*CHANNLE_EACH_GROUP)
                    input_split = input[0][:, affine_idx: affine_idx+CHANNLE_EACH_GROUP, :, :] # [B, C_split, H, W]
                    weight_split = module.weight.data[:, affine_idx: affine_idx+CHANNLE_EACH_GROUP, :, :] # [Cout, Cin_split, K, K]
                    output_split = F.conv2d(input_split, weight_split, None, module.stride, module.padding,
                                      module.dilation, module.groups) # [B, Cout, H, W]
                    output_split_sum += output_split


                    hist_y = (torch.histc(input=output_split.data.cpu(), bins=100) / output_split.data.numel()).cpu()  # 百分比直方图
                    hist_x = np.array([(output_split.max() - output_split.min()) / 100 * i + output_split.min() for i in range(100)])
                    plt.plot(hist_x, hist_y, color="red")
                    save_path = os.path.join(HIST_SAVE_DIR, str(module.conv_idx))
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    save_path = os.path.join(save_path, "batch_" + str(module.number_of_accumulate_batchs) + "_group_" + str(i) + ".png")
                    plt.savefig(save_path)
                    plt.clf()

                if incomplete_group_size != 0:
                    input_split = input[0][:, -1*incomplete_group_size:, :, :]  # [B, incomplete_group_size, H, W]
                    weight_split = module.weight.data[:, -1*incomplete_group_size:, :,
                                   :]  # [Cout, incomplete_group_size, K, K]
                    output_split = F.conv2d(input_split, weight_split, None, module.stride, module.padding,
                                            module.dilation, module.groups)  # [B, Cout, H, W]
                    output_split_sum += output_split

                    hist_y = (torch.histc(input=output_split.data.cpu(),
                                          bins=100) / output_split.data.numel()).cpu()  # 百分比直方图
                    hist_x = np.array(
                        [(output_split.max() - output_split.min()) / 100 * i + output_split.min() for i in range(100)])
                    plt.plot(hist_x, hist_y, color="red")
                    save_path = os.path.join(HIST_SAVE_DIR, str(module.conv_idx))
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    save_path = os.path.join(save_path,
                                             "batch_" + str(module.number_of_accumulate_batchs) + "_group_" + str(
                                                 group_number) + ".png")
                    plt.savefig(save_path)
                    plt.clf()

                output_split_sum = output_split_sum + module.bias.data.reshape(1, -1, 1, 1)

                difference = output_split_sum - output

                print("difference.abs().max():{}".format(difference.abs().max()))
                ...
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                save_path = os.path.join(save_path, "batch_" + str(module.number_of_accumulate_batchs) + "_sum.png")
                plt.savefig(save_path)
                plt.clf()
...
def get_args():
    ...
    # args for conv split
    parser.add_argument('--channel_each_group', type=int, nargs='?', default=-1,
                        help='-1:use original conv, others integars > 0: number of channel in each group')
    parser.add_argument('--hist_save_dir', type=str, default="./hist",
                        help="path to save histogram")
    parser.add_argument('--BN_hook', action='store_true',
                        help='if true, use hook in BN to calculate feature map histogram')
...
if __name__ == "__main__":
    ...
    CHANNLE_EACH_GROUP = int(args.channel_each_group)
    HIST_SAVE_DIR = args.hist_save_dir

    if not os.path.exists(HIST_SAVE_DIR):
        os.makedirs(HIST_SAVE_DIR)

    # add conv_id for each convolution layer
    conv_idx = 0
    if not args.BN_hook:
        for name, module in net.named_modules():
            if isinstance(module, nn.Conv2d):
                module.conv_idx = conv_idx
                conv_idx += 1
    elif args.BN_hook:
        for name, module in net.named_modules():
    ...
    # register hook for calculate feature map histogram
    handle_feature_map_list = []
    if not args.BN_hook:
        for name, layer in net.named_modules():
            if isinstance(layer, nn.Conv2d):
                handle = layer.register_forward_hook(hook_conv_results_histogram)
                handle_feature_map_list.append(handle)
                print("{} register hook".format(name))
    elif args.BN_hook:
        for name, layer in net.named_modules():
    ...
    for handle in handle_feature_map_list:
        handle.remove()
