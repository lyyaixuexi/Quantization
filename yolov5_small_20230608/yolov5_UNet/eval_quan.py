...
import UNet.tflite_quantization_PACT_weight_and_act as tf
import UNet.model_transform as mt
import numpy as np
from UNet.utils.model_analyse import ModelAnalyse
import os
from load_quan_scale_param import load_param
from load_quan_scale_param import load_scale
...
def hook_conv_results_checkoverflow(module, input, output):
    ...
    if isinstance(module, tf.Conv2d_quantization):
        # pred_scale = (module.act_scale * module.weight_scale).unsqueeze(0).unsqueeze(2).unsqueeze(3).expand_as(
        #     output)
        clamp_range = c_round(2 ** (c_round(module.weight_bits) - 1) - 1)
        pred_scale = module.pred_max / clamp_range
        
    # output = tf.c_round(output.detach() / pred_scale)
    output=module.tmp_int_output
...
def convert_to_full_int(net, device):
    ...
    next_act_list = tf.layer_transform(net)
...
def run(data,
    ...
    # m_b=12
    # bias_b=16
    # model = tf.replace(model=model, quantization_bits=q_b, m_bits=m_b, bias_bits=bias_b, inference_type="full_int", Mn_aware=False,
    #                              fuseBN=False)
    # model = tf.fuse_doubleconv(model)
    for name, layer in model.named_modules():
        if isinstance(layer, (tf.Conv2d_quantization)):
            layer.inference_type="full_int"
            layer.convert_fp32_to_int = True

    # model = tf.open_Mn(model, m_b)
    ...
    # load quan_scale and quan_param
    

    print(model)
    
    input_scale = None
    model, input_scale=tf.layer_transform(model)
    # input_scale = input_scale.to(device)

    if save_quantized_layer:
        model.to(device='cpu')
        # result_name="output_full_int/output_w4a4b16"
        quantized_param_file = result_name + '_param'
        scale_table_file = result_name + '.scale'
        ...
        assert False, "finish save, so terminate!!!"
...
def parse_opt():
    ...
    parser.add_argument('--result_name', type=str, default="", help='results path of save_quantized_layer')
