...
# # hook setting
# def register_hook_every_conv(model, func):
#     # 注册钩子，用于获取每层卷积的输入和输出
#     handle_list_every_conv = []
#     for name, layer in model.named_modules():
#         if isinstance(layer, nn.Conv2d):
#             handle = layer.register_forward_hook(func)
#             handle_list_every_conv.append(handle)
#
#     return handle_list_every_conv
#
# every_conv_result = []
# def hook_every_conv_results(module, input, output):
#     # 获取每层卷积的输出
#     every_conv_result.append(output)
...
def train_net(net,
    ...
    if dist.get_rank() == 0:
        writer = SummaryWriter(log_dir=args.checkpoint, comment=f'LR_{lr}_BS_{batch_size}')
    ...
    if (args.OAQ_m != 0):
        criterion_for_overflow = OverflowActivationLoss()
    ...
    if dist.get_rank() == 0:
        # handle_list_every_conv = register_hook_every_conv(net.module, hook_every_conv_results)

        mIoU = eval_net(net=net.module, loader=val_loader, device=local_rank, input_scale=1)

        # for handle in handle_list_every_conv:
        #     handle.remove()
        # every_conv_result.clear()
    ...
    for epoch in range(epochs):
        ...
        for image, targetmask in train_loader:
            ...
            masks_pred = net(imgs)
            task_loss = criterion(masks_pred, true_masks)

            if (args.OAQ_m!=0) and (global_step%args.OAQ_m==0): # todo
                overflow_element_list = get_overflow_element(local_rank, net.module, oaq_conv_result, logging)
                overflow_loss = criterion_for_overflow(overflow_element_list=overflow_element_list, batch_size=batch_size)

                for handle in oaq_handle_list:
                    handle.remove()
                oaq_conv_result.clear()

                # print("overflow_loss:{}. task_loss:{}".format(overflow_loss, task_loss))

                loss = overflow_loss*args.beta + task_loss
            else:
                loss = task_loss
            ...
            if dist.get_rank() == 0:
                writer.add_scalar('Task_Loss_batch/train', task_loss.item(), global_step)

                if (args.OAQ_m != 0) and (global_step % args.OAQ_m == 0):
                    writer.add_scalar('Overflow_Loss_batch/train', overflow_loss.item(), global_step)
            ...
            # # test todo
            # print(net.module.down1.maxpool_conv[1].double_conv[0].weight.grad[0][0])
...
def eval_net(net, loader, device, input_scale, inference_type=None):
    ...
    with tqdm(total=n_val, desc='Validation round', unit='batch', leave=False) as pbar:
        for image, targetmask in loader:
            ...
            # for feature in every_conv_result:
            #     print("mean:{}, min:{}, max:{}".format(feature.mean(), feature.min(), feature.max()))
            #     area_intersect = torch.histc(feature, bins=10, min=-100, max=100)
            #     element_number = feature.numel()
            #     print("histc:{}".format(area_intersect/element_number*100))
            #
            # print("clear feature list")
            # every_conv_result.clear()
...
def get_args():
    ...
    parser.add_argument('--beta', type=float, default=1.0, help="to balance the overflow loss and task-specific loss")
    ...
    parser.add_argument('--jointly', action='store_true',
                        help='if true, train quantize model with downscaling & overflow-aware training jointly ')
