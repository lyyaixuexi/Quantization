...
# sys.path.append(r'D:\\Study\\谭明奎实验室\\训练Yolov5\\new_try_yolov5_unet\\yolov5_UNet\\UNet\\unet')
# sys.path.append(r'D:\\Study\\谭明奎实验室\\训练Yolov5\\new_try_yolov5_unet\\yolov5_UNet\\UNet\\utils')

# print(sys.path)



# sys.path.append(r'D:\\Study\\谭明奎实验室\\训练Yolov5\\new_try_yolov5_unet\\yolov5_UNet\\UNet')
# sys.path.append(r'D:\\Study\\谭明奎实验室\\训练Yolov5\\new_try_yolov5_unet\\yolov5_UNet\\UNet\\unet')
# sys.path.append(r'D:\\Study\\谭明奎实验室\\训练Yolov5\\new_try_yolov5_unet\\yolov5_UNet\\UNet\\utils')
from UNet import tflite_quantization_PACT_weight_and_act
from UNet import train
from UNet.train import register_hook
# from UNet.train import hook_conv_results
from UNet.train import hook_conv_results_checkoverflow
from UNet.overflow_utils import calculate_No
from UNet.overflow_utils import update_alpha
import logging
...
from utils.general import (LOGGER, check_dataset, check_file, check_git_status,
                           check_img_size, check_requirements, check_suffix, check_yaml,
                           colorstr, get_latest_run, increment_path, init_seeds,
...
def train(hyp,               # path/to/hyp.yaml or hyp dictionary
    ...
    (w.parent if evolve else w).mkdir(parents=True,
    ...
    names = ['item'] if single_cls and len(data_dict['names']) != 1 else data_dict[
        'names']               # class names
    assert len(
        names
    ...
    if pretrained:
        ...
        LOGGER.info(
            'No pruned weights loaded, please set the right pruned weight path ...'
        )               # report
        return

    # 替换最后一层网络（Detect模块的最后一个1*1卷积）为FC量化层

    # print("Starting to print model:\n\n")

    # print(model)

    # print("\n\nPrint over, the model is above.\n\n")
    ...
    #         print(module,'\n')
    #         # print(isinstance(module.m[-1], torch.nn.Conv2d), '\n')
    #         module_list=module.m
    #         module_list[-1]=torch.nn.Conv2d(225, 30, kernel_size=(1,1))
    #         print(module,'\n')
    ...
    model = tflite_quantization_PACT_weight_and_act.replace(model=model, Mn_aware=True)
    ...
    # print(model)
    handle_list = register_hook(model, hook_conv_results_checkoverflow)

    # device = torch.device("cuda:0")
    
    # print(model)

    freeze_list = ['model.24.m.2.weight', 'model.24.m.2.bias', 'model.24.m.2.act_max']
    # Freeze
    ...
    # if opt.adam:
    #     optimizer = AdamW(g1, lr=hyp['lr0'])  # adjust beta1 to momentum
    # else:
    #     optimizer = SGD(g1, lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)

    # if opt.adam:
    #     optimizer = AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=hyp['lr0'])  # adjust beta1 to momentum
    # else:
    #     optimizer = SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)
    
    # if opt.adam:
    #     optimizer = AdamW(model.parameters(), lr=hyp['lr0'])  # adjust beta1 to momentum
    # else:
    #     optimizer = SGD(model.parameters(), lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)


    optimizer = torch.optim.RMSprop(filter(lambda p: p.requires_grad, model.parameters()), lr=0.000001, weight_decay=1e-8, momentum=0.9)
    # optimizer = torch.optim.RMSprop(model.parameters(), lr=0.1, weight_decay=1e-8, momentum=0.9)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if(len(oaq_conv_result)>0):
                max_oaq_conv_result=torch.max(oaq_conv_result[0])
                max_oq.append(max_oaq_conv_result)
            ...
            #    # print('-->grad_value:',parms.grad)
            #    # print("==========================")
            #     # print('{} has update {}'.format(name, torch.sum(parms-par_0[i])))
            #     # print('{} has update {}'.format(name, parms-par_0[i]))
            ...
            if (OAQ_m!=0) and (global_step%OAQ_m==0):
                with torch.no_grad():
                    ...
                    lr_max = 0.1
                    lr_curr = optimizer.param_groups[0]['lr']
                    update_alpha(device, model, No, batch_size, lr_max, lr_curr, logging)
    ...
    # torch.save({"model":model}, "pruned_model_8bits.pt")

    from utils.torch_utils import model_info
    model_info(model, img_size=416)
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {
            ...
            'anchors': (2, 2.0,
                        10.0),               # anchors per output grid (0 to ignore)
            'fl_gamma':
            (0, 0.0,
             2.0),               # focal loss gamma (efficientDet default gamma=1.5)
            'hsv_h': (1, 0.0, 0.1),               # image HSV-Hue augmentation (fraction)
            'hsv_s': (1, 0.0,
                      0.9),               # image HSV-Saturation augmentation (fraction)
            'hsv_v': (1, 0.0,
                      0.9),               # image HSV-Value augmentation (fraction)
            ...
            'perspective':
            (0, 0.0,
             0.001),               # image perspective (+/- fraction), range 0-0.001
