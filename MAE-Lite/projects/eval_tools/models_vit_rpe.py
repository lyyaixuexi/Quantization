...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, with_attn=False):
        B, N, C = x.shape
        qkv_bias = None
...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, with_hidden=False, with_attn=False):
        if self.gamma_1 is None:
            shortcut = x
            x = self.norm1(x)
            hidden = x if with_hidden else None
            x, attn = self.attn(x, rel_pos_bias=rel_pos_bias, with_attn=with_attn)
...
@register_model
def vit_tiny_patch16_rpe(pretrained=False, global_pool=True, **kwargs):
    # for fine-tuning
    defaults = {
