    ...
    for name, module in net.named_modules():
        ...
        # if(name=='model.24.m.2'):
            ...
            if module.classify_layer:
                module.pred_max = nn.Parameter(torch.ones(1)*279)
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        if self.Mn_aware:
            ...
            if self.classify_layer:
                ...
                # self.pred_max = nn.Parameter(torch.ones(1)*29) # 4bit
                self.pred_max = nn.Parameter(torch.ones(1) * 40) # 8bit 16bit
    ...
    def forward(self, input):
        ...
        if self.inference_type == "all_fp":
            ...
            if self.fuseBN:
                ...
                # print("quantized_input", quantized_input.dtype)
                ...
                if self.bias is not None:
                    ...
                    # print("quantized_weight", quantized_weight.dtype)
                    # not quantization self.bias
                    quantized_bias = None
        ...
        if self.inference_type == "full_int":
            ...
            int_output = F.conv2d(int_input, self.int_weight, None, self.stride, self.padding,
                                                        self.dilation, self.groups)
        ...
        for name, module in model.named_modules():
            ...
            if(name=='conv9'):
                if isinstance(module, nn.Conv2d):
                    ...
                    classify_layer=True,
                    fuseBN=False if module.out_channels==279 else fuseBN,
                    Mn_aware=Mn_aware)
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Conv2d):
                ...
                temp_conv = Conv2d_quantization(
                    ...
                    bias_bits=bits_bias,
                    inference_type=inference_type,
                    classify_layer=True if module.out_channels == 279 else False,
                    fuseBN=False if module.out_channels == 279 else fuseBN,
                    Mn_aware=Mn_aware
                )
                temp_conv.weight.data.copy_(module.weight.data)
