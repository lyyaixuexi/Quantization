...
class USBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, expand=1.0):
        ...
        self.bn1 = USBatchNorm2d(planes, expand=expand,former_conv_kernel_size=3)

        self.conv2 = conv3x3(planes, planes, expand=expand)
        self.bn2 = USBatchNorm2d(planes, expand=expand,former_conv_kernel_size=3)
    ...
    def non_uniform_set_width(self, ch_in, candidate):
        #print('USBasicBlock non_uniform_set_width:{}'.format(ch_in))
        ...
        return ch_out
    def uniform_set_width(self, ch_in):
        #print('USBasicBlock uniform_set_width:{}'.format(ch_in))
        self.conv1.set_input_width(specific_ch=ch_in)
        #_, ch_out = self.conv1.random_sample_output_width(candidate)
        self.conv1.set_output_width(width=1.0)
        ch_out=self.conv1.cur_out_ch
        ...
        if self.downsample is None:
            ...
            #_, ch_out = self.conv2.random_sample_output_width(candidate)
            self.conv2.set_output_width(width=1.0)
...
class USBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, expand=1.0):
        ...
        self.bn1 = USBatchNorm2d(planes, expand=expand,former_conv_kernel_size=1)
        self.conv2 = USConv2d(planes, planes, kernel_size=3, stride=stride,
                              padding=1, bias=False, expand=expand)
        self.bn2 = USBatchNorm2d(planes, expand=expand,former_conv_kernel_size=3)
        self.conv3 = USConv2d(planes, planes * 4, kernel_size=1,
                              bias=False, expand=expand)
        self.bn3 = USBatchNorm2d(planes * 4, expand=expand,former_conv_kernel_size=1)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class USResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, input_size=224, max_width=1.0):
        ...
        self.bn1 = USBatchNorm2d(self.inplanes, expand=max_width,former_conv_kernel_size=7)
        self.relu = nn.ReLU(inplace=True)
        ...
        # self.conv_out = nn.Conv2d(in_channels=448, out_channels=279, kernel_size=1)
    ...
    def uniform_set_width(self, width):
        #print('us_resnet:uniform_set_width')
        ##print(width)
        self.conv1.set_output_width(width=width)
        ch_out=self.conv1.cur_out_ch
        self.bn1.set_output_width(specific_ch=ch_out) 
        for m in self.modules():
            if isinstance(m, (USBasicBlock,USBottleneck)):
                ch_out=m.uniform_set_width(ch_out)
        ...
        '''        
        for m in self.modules():
            if isinstance(m,USBasicBlock):
                m.update_channels()
        '''
 
    def non_uniform_set_width(self, candidate):
        #print('us_resnet:non_uniform_set_width')
        _, ch_out = self.conv1.random_sample_output_width(candidate)
        self.bn1.set_output_width(specific_ch=ch_out)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                USBatchNorm2d(planes * block.expansion, expand=self.max_width,former_conv_kernel_size=1),
            )
