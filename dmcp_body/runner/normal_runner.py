...
from utils.meter import AverageMeter, accuracy, calc_adaptive_model_flops, calc_model_parameters
        ...
        for e in range(self.cur_epoch, self.config.training.epoch):
            ...
            for batch_idx, (x, y) in enumerate(train_loader):
                ...
                if self.cur_step >= self.config.validation.start_val and self.cur_step % self.config.validation.val_freq == 0:
                    print(
                        'self.cur_step >= self.config.validation.start_val and self.cur_step % self.config.validation.val_freq == 0')
    ...
    def validate(self, val_loader, tb_logger=None):
        ...
        with torch.no_grad():
            for batch_idx, (x, y) in enumerate(val_loader):
                ...
                top1, top5, prec1_class = accuracy(out, y, top_k=(1, 5))  ##############################
