...
def get_data_loader(config):
    ...
    elif config.dataset.type == 'PCB':
        dataset = data.get_pcb(config.dataset)
    elif config.dataset.type == 'TSR':
        dataset = data.get_tsr(config.dataset)
    else:
...
def setDDP(model,args):
    ...
    return model
def get_model(args,config, checkpoint=None):
    ...
    if config.model.runner.type == 'USRunner':
        ...
        runner = DMCPRunner(args,config, wrapper)
    elif config.model.runner.type == 'NormalRunner':
        runner = NormalRunner(args,config, wrapper)
    else:
        raise NotImplementedError
