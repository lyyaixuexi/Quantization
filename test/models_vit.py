...
from timm.models.vision_transformer import PatchEmbed, DropPath, Mlp, trunc_normal_, lecun_normal_, Attention as BaseAttn
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.fw = open("/mnt/cephfs/home/lyy/Quantization/test/test.txt", 'a')
    def forward(self, x):
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        self.fw.write('{:.2f}'.format(curr_time))
        self.fw.write('\n')
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, global_pool=False, weight_init=''):
        """
        Args:
        ...
        self.fw = open("/mnt/cephfs/home/lyy/Quantization/test/test.txt", 'w')
        self.fw.write('')
    ...
    def forward_features(self, x):
        ...
        # print("7: ", x.dtype)
        if self.dist_token is None:
            # print("8: ", self.pre_logits(x[:, 0]).shape)
            return x
            # return self.pre_logits(x[:, 0])
        else:
            return x[:, 0], x[:, 1]
