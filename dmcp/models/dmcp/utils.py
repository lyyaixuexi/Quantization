...
def dump_flops_stats(iteration, config, model):
    #print('utils:dump_flops_stats')
    sample_flops = []
    num_sample = config.arch.num_flops_stats_sample
    
    #alpha_training=model.module.alpha_training
    #model.module.set_alpha_training(False)
    for i in range(num_sample):
        #print(i)
        #print('utils: model.module.direct_sampling()')
    ...
    #model.module.set_alpha_training(alpha_training)
...
def sample_model(config, model):
    ...
    for i in range(num_sample + 1):
        ...
        while cur_flops > config.arch.target_flops * 1.3 or cur_flops < config.arch.target_flops * 0.6:
            model.module.direct_sampling()
            cur_flops = calc_model_flops(model, config.dataset.input_size)
